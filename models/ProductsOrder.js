...
class productsOrderManager {
  ...
  static async getAllProducts() {
      const queryResponse = await pgClient.query("select * from products_in_order");
      const ProductsOrder = convertOrderDataToObjects(queryResponse.rows);
      return ProductsOrder;
    }
  static async getId(id) {
      const queryResponse = await pgClient.query(" select * from products_in_order WHERE id_order=$1",[id])
      const ProductsOrder = convertOrderDataToObjects(queryResponse.rows);
      return ProductsOrder;
  }
  //Relacion
 // insert into orders (order_date,order_mail) values ('03-02-2023','gj');
//insert into products_in_order (id_order,id_product,units,price) values ((select max(id_num_order) from orders),1,1,8.5),((select max(id_num_order) from orders),2,1,4.25);
//insert into order_status(id_order,id-status) values select max(id_num_order) from orders),1);
